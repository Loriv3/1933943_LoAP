...
async def get_metrics_list(websocket: WebSocket):
    try:
        async with httpx.AsyncClient() as httpx_client:
            cache_data = await httpx_client.get(f"{CACHE_URL}/api/metrics")
...
async def get_metrics_group(websocket: WebSocket, group_id: str):
    ...
    try:
        async with httpx.AsyncClient() as httpx_client:
            cache_data = await httpx_client.get(f"{CACHE_URL}/api/metrics/{group_id}")
            logger.info(f"Got cache response for {group_id}: {cache_data}")
        await manager.connect(websocket, group_id, cache_data)
        while True:
